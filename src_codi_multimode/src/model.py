...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # Direct answer task
    direct_loss_factor: float = field(default=1.0, metadata={"help": "Weight for direct answer loss (δ)"})
    train_direct_answer: bool = field(default=True, metadata={"help": "Include direct answer task in training"})
    # Inference settings
    inference_mode: str = field(default="latent", metadata={"help": "Inference mode: direct, verbalized, or latent"})
...
class CODI(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        # Load tokenizer to check for existing <|eot_id|> token
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, use_fast=False)

        # Control token scheme:
        # - eot_id: end of prompt (only for instruction-tuned models like Llama)
        # - bocot_id: begin chain-of-thought
        # - eocot_id: end chain-of-thought

        # Check if <|eot_id|> exists in tokenizer (Llama 3.2 has it)
        eot_token = self.tokenizer.convert_tokens_to_ids("<|eot_id|>")
        if eot_token != self.tokenizer.unk_token_id:
            # Llama: <|eot_id|> exists and has semantic meaning, reuse it
            self.eot_id = eot_token
            self.use_eot_id = True
            self.bocot_id = ori_vocab_size
            self.eocot_id = ori_vocab_size + 1
            num_new_tokens = 2
        else:
            # GPT-2: no instruction tuning, eot_id not needed
            self.eot_id = None
            self.use_eot_id = False
            self.bocot_id = ori_vocab_size
            self.eocot_id = ori_vocab_size + 1
            num_new_tokens = 2

        # Keep bot_id as alias for bocot_id for backward compatibility
        self.bot_id = self.bocot_id

        self.codi.resize_token_embeddings(ori_vocab_size + num_new_tokens)

        self.dim = self.codi.config.hidden_size
        self.num_latent = training_args.num_latent
        # tokenizer already initialized above for token detection

        # LoRA
        if training_args.use_lora:
            self.codi = get_peft_model(self.codi, lora_config)

            # Unfreeze new token embeddings so model can learn to generate them
            # This is necessary for verbalized mode which must output eocot_id
            embed_layer = self.get_embd(self.codi, self.model_name)
            embed_layer.weight.requires_grad = True

            # Also unfreeze lm_head (needed to output new tokens)
            # Note: for some models lm_head is tied to embeddings, but we set both to be safe
            lm_head = self.codi.get_base_model().lm_head
        ...
        # Direct Answer Loss
        self.direct_loss_factor = training_args.direct_loss_factor
        self.train_direct_answer = training_args.train_direct_answer
    ...
    def forward(
        ...
        # New inputs for direct answer
        direct_input_ids: torch.LongTensor = None,  # prompt + eot_id + answer (no bot_id)
        direct_labels: torch.LongTensor = None,     # labels for direct answer
        direct_attention_mask: Optional[torch.LongTensor] = None,
        step: int = None,
        ...
        # Calculate the CE loss for direct answer task
        direct_ce_loss = 0
        if direct_input_ids is not None and self.train_direct_answer:
            direct_outputs = self.codi(input_ids=direct_input_ids, output_hidden_states=True, attention_mask=direct_attention_mask)
            direct_logits = direct_outputs.logits
            effective_direct_logits = direct_logits[:, :-1, :].reshape(-1, direct_logits.size(-1))
            direct_target_ids = direct_labels[:, 1:].reshape(-1)
            direct_ce_loss = self.loss_fct(effective_direct_logits, direct_target_ids)
            direct_ce_loss *= self.direct_loss_factor
        ...
        if self.print_loss:
            print(f'loss={ce_loss+distill_loss}, ce_loss={ce_loss}, distill_loss={distill_loss}, ce_loss_total={ce_loss_total}, distill_loss_total={distill_loss_total}, ref_ce_loss={ref_ce_loss}, direct_ce_loss={direct_ce_loss}')

        loss = ce_loss_total + distill_loss_total + ref_ce_loss + direct_ce_loss
        ...
        if direct_ce_loss != 0:
            direct_ce_loss = direct_ce_loss.detach().item()

        return {"loss": loss, "logits": logits, "ce_loss": ce_loss_total, "distill_loss": distill_loss_total, "ref_ce_loss": ref_ce_loss, "direct_ce_loss": direct_ce_loss}
